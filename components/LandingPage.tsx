...
const LandingPage: React.FC<LandingPageProps> = ({ onStart, onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-brand-dark font-sans text-brand-dark dark:text-white transition-colors selection:bg-brand-violet/30 overflow-x-hidden">
        ...
        <div className="bg-white/90 dark:bg-brand-dark/95 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-full px-4 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between shadow-2xl">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <button 
              type="button"
              onClick={() => onLogin()}
              className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-brand-violet transition-colors px-4 py-3 cursor-pointer relative z-[60] touch-manipulation active:bg-slate-100 dark:active:bg-white/10 rounded-lg"
            >
              Entrar
            </button>
            <button 
              type="button"
              onClick={() => onStart()}
              className="bg-brand-violet text-white px-4 sm:px-6 py-2.5 sm:py-2.5 rounded-full text-[10px] sm:text-xs font-black shadow-lg hover:bg-purple-600 active:scale-95 transition-all whitespace-nowrap relative z-[60] touch-manipulation"
            >
              COMEÇAR
            </button>
          </div>
      ...
      <section id="instalar" className="py-32 bg-gradient-to-b from-brand-violet/5 to-white dark:from-brand-violet/10 dark:to-brand-dark">
         <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-16">
               <div className="inline-flex items-center gap-2 bg-brand-violet/10 text-brand-violet px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider mb-6">
                  <Smartphone size={14} /> Acesso Rápido
               </div>
               <h2 className="text-4xl sm:text-5xl font-black tracking-tighter mb-4">Instale o App no seu Celular</h2>
               <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto text-lg">Tenha o Espiritualizei sempre à mão, como um app nativo, sem precisar baixar da loja.</p>
            </div>

            <div className="grid md:grid-cols-2 gap-8">
               {/* iOS */}
               <div className="bg-white dark:bg-white/5 rounded-[2rem] p-8 border border-slate-100 dark:border-white/10 shadow-xl">
                  <div className="flex items-center gap-4 mb-6">
                     <div className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center">
                        <svg viewBox="0 0 24 24" fill="white" className="w-8 h-8"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
                     </div>
                     <div>
                        <h3 className="text-xl font-black">iPhone / iPad</h3>
                        <p className="text-sm text-slate-500">Safari</p>
                     </div>
                  </div>
                  <div className="space-y-4">
                     <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center shrink-0 font-bold text-sm">1</div>
                        <p className="text-sm text-slate-600 dark:text-slate-300">Abra o site no <strong>Safari</strong> e toque no ícone de <strong>Compartilhar</strong> (quadrado com seta para cima)</p>
                     </div>
                     <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center shrink-0 font-bold text-sm">2</div>
                        <p className="text-sm text-slate-600 dark:text-slate-300">Role para baixo e toque em <strong>"Adicionar à Tela de Início"</strong></p>
                     </div>
                     <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-brand-violet/10 text-brand-violet flex items-center justify-center shrink-0 font-bold text-sm">3</div>
                        <p className="text-sm text-slate-600 dark:text-slate-300">Confirme tocando em <strong>"Adicionar"</strong> e pronto!</p>
                     </div>
                  </div>
               </div>
               ...
               <div className="bg-white dark:bg-white/5 rounded-[2rem] p-8 border border-slate-100 dark:border-white/10 shadow-xl">
                  <div className="flex items-center gap-4 mb-6">
                     <div className="w-14 h-14 bg-green-500 rounded-2xl flex items-center justify-center">
                        <svg viewBox="0 0 24 24" fill="white" className="w-8 h-8"><path d="M17.6 9.48l1.84-3.18c.16-.31.04-.69-.26-.85-.29-.15-.65-.06-.83.22l-1.88 3.24a11.463 11.463 0 00-8.94 0L5.65 5.67c-.19-.29-.58-.38-.87-.2-.28.18-.37.54-.22.83L6.4 9.48A10.78 10.78 0 003 18h18a10.78 10.78 0 00-3.4-8.52zM7 15.25a1.25 1.25 0 110-2.5 1.25 1.25 0 010 2.5zm10 0a1.25 1.25 0 110-2.5 1.25 1.25 0 010 2.5z"/></svg>
                     </div>
                     <div>
                        <h3 className="text-xl font-black">Android</h3>
                        <p className="text-sm text-slate-500">Chrome</p>
                     </div>
                  </div>
                  <div className="space-y-4">
                     <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center shrink-0 font-bold text-sm">1</div>
                        <p className="text-sm text-slate-600 dark:text-slate-300">Abra o site no <strong>Chrome</strong> e toque nos <strong>três pontinhos</strong> (menu)</p>
                     </div>
                     <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-green-500/10 text-green-500 flex items-center justify-center shrink-0 font-bold text-sm">2</div>
                        <p className="text-sm text-slate-600 dark:text-slate-300">Selecione <strong>"Instalar aplicativo"</strong> ou <strong>"Adicionar à tela inicial"</strong></p>
                     </div>
                     <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-brand-violet/10 text-brand-violet flex items-center justify-center shrink-0 font-bold text-sm">3</div>
                        <p className="text-sm text-slate-600 dark:text-slate-300">Confirme a instalação e o ícone aparecerá na sua tela!</p>
                     </div>
                  </div>
               </div>
            </div>
