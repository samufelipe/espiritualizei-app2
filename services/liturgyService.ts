...
export const generateWeeklyLibraryContent = async () => {
  const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
  const seasonInfo = getSeasonDetailedInfo(new Date());

  const prompt = `
    Você é um especialista em teologia católica e liturgia. 
    Gere um conteúdo profundo e cativante para a biblioteca de um aplicativo espiritual chamado "Espiritualizei".
    
    O conteúdo deve ser baseado no tempo litúrgico atual: ${seasonInfo.name} (${seasonInfo.theme}).
    Hoje é ${new Date().toLocaleDateString('pt-BR')}.

    Retorne um objeto JSON com a seguinte estrutura:
    {
      "title": "Título impactante e poético",
      "description": "Uma breve descrição que instigue a curiosidade (máx 150 caracteres)",
      "content": "O texto completo em Markdown, com introdução, desenvolvimento e conclusão. Use um tom de proximidade, como um diretor espiritual falando com um jovem peregrino. Mínimo 500 palavras.",
      "category": "doctrine",
      "duration": "12 min",
      "icon_name": "Flame",
      "video_suggestion": {
        "title": "Título de um vídeo real no YouTube sobre o tema",
        "url": "https://www.youtube.com/results?search_query=tema+catolico",
        "channelName": "Canal Católico"
      },
      "music_suggestion": {
        "title": "Música Católica",
        "url": "https://www.youtube.com/results?search_query=musica+catolica",
        "artist": "Artista"
      },
      "deepen_actions": [
        { "label": "Fazer um exame de consciência", "type": "meditation" },
        { "label": "Rezar o terço", "type": "prayer" },
        { "label": "Leitura Espiritual", "type": "reading" }
      ]
    }
  `;
  ...
  try {
    ...
    if (!jsonMatch) throw new Error("Falha ao gerar JSON");
...
export const saveWeeklyLibraryContent = async (content: any) => {
  if (!supabase) return null;
  const id = `weekly-${Date.now()}`;
  const { data, error } = await supabase.from('knowledge_items').insert([{
    id,
    track_id: 'track-weekly-highlights',
    title: content.title,
    ...
    video_suggestion: content.video_suggestion,
    music_suggestion: content.music_suggestion,
    deepen_actions: content.deepen_actions,
    is_featured: true,
