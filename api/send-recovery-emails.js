/**
 * Vercel API — trigger manual para process-recovery-emails
 * O disparo automático a cada 10 minutos é feito via Supabase pg_cron.
 * Esta rota existe para testes e reprocessamento manual.
 */
...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/process-recovery-emails`,
      {
        method:  'POST',
        headers: {
    ...
    console.error('[send-recovery-emails]', e.message)
