...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/create-pix-checkout`,
      {
        method: 'POST',
        headers: {
    ...
    console.error('[pix-checkout proxy]', error.message)
    return res.status(500).json({ error: 'Servidor de pagamento indisponivel. Tente novamente.' })
  }
}
